...
In this document, youâ€™ll learn about shipping options and their rules.
...
You can restrict shipping options by custom rules, such as the itemâ€™s weight or the customer group.

You can also restrict a shipping option's price based on specific conditions. For example, you can make a shipping option's price free based on the cart total. Learn more in the Pricing Module's [Price Rules](https://docs.medusajs.com/commerce-modules/pricing/price-rules#how-to-set-rules-on-a-price) guide.
...
- `operator`: The operator used in the condition. For example:
    - To allow multiple values, use the operator `in`, which validates that the provided values are in the ruleâ€™s values.
    - To create a negation condition that considers `value` against the rule, use `nin`, which validates that the provided values arenâ€™t in the ruleâ€™s values.
